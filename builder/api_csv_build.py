...
def convert_db_row_to_api_format(row: dict) -> dict:
    ...
    result = {
        ...
        "boundarySourceURL": (
            row.get("boundarysourceurl", "")
            .replace("https//", "")
            .replace("http//", "")
            .replace("https://", "")
            .replace("http://", "")
        ),
        "sourceDataUpdateDate": row.get("sourcedataupdatedate", "").strftime(
            "%a %b %d %H:%M:%S %Y"
        )
        if row.get("sourcedataupdatedate")
        else "",
        "buildDate": row.get("builddate", "").strftime("%b %d, %Y")
    ...
    try:
        with connect_to_db() as conn:
            with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
                ...
                cur.execute("SELECT * FROM boundary_meta")
                all_rows = [
                    convert_datetime_to_iso(convert_db_row_to_api_format(row))
                ...
                for adm in adm_levels:
                    cur.execute(
                        "SELECT * FROM boundary_meta WHERE boundarytype = %s",
                        (adm.upper(),),
                    )
                    adm_rows = [
                        convert_datetime_to_iso(convert_db_row_to_api_format(row))
                ...
                cur.execute(
                    "SELECT DISTINCT boundaryiso FROM boundary_meta WHERE boundaryiso IS NOT NULL"
                )
                iso_rows = cur.fetchall()
                all_isos = [
                    row["boundaryiso"]
                    for row in iso_rows
                    if row and "boundaryiso" in row and row["boundaryiso"]
                ]

                if not all_isos:
                    with log_lock:
                        logging.warning(
                            "No boundaryISO values found in boundary_meta table"
                        )
                ...
                for iso in all_isos:
                    try:
                        ...
                        cur.execute(
                            "SELECT * FROM boundary_meta WHERE boundaryiso = %s", (iso,)
                        )
                        rows = cur.fetchall()
                        if not rows:
                            continue
                            ...
                            try:
                                ...
                                adm_row = next(
                                    (
                                        row
                                        for row in rows
                                        if row.get("boundarytype", "").upper() == adm
                                    ),
                                    None,
                                )
                                ...
                                adm_data = convert_datetime_to_iso(
                                    convert_db_row_to_api_format(adm_row)
                                )
                                ...
                                with log_lock:
                                    logging.error(
                                        f"Error processing {iso}/{adm}: {str(adm_error)}"
                                    )
