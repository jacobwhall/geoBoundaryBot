...
class builder:
    ...
    def dataLoad(self):
        ...
        if len(allShps) == 1:
            if len(shp) == 1:
                ...
                try:
                    self.geomDta = gpd.read_file(
                        os.path.join(self.zipExtractPath, shp[0])
                    )
                    self.dataLoadFail = 0
                except Exception as e:
            ...
            if len(geojson) == 1:
                ...
                try:
                    self.geomDta = gpd.read_file(
                        os.path.join(self.zipExtractPath, geojson[0])
                    )
                    self.dataLoadFail = 0
                except Exception as e:
            ...
            self.logger.critical(
                "There was more than one geometry file (shapefile or geojson) present in the zipfile."
            )
    ...
    def checkSourceValidity(self):
        ...
        if self.metaExistsFail == 1:
            return (
                "ERROR: There is no meta.txt in the source zipfile for this boundary."
            )
    ...
    def checkBuildTabularMetaData(self):
        ...
        for m in self.metaData.splitlines():
                ...
                try:
                    if "to" in val:
                        ...
                        if (year > 1950) and (year <= datetime.datetime.now().year):
                            ...
                            self.metaReq["year"] = (
                                "ERROR: The year provided in the metadata was invalid (Not numerically what we expected): "
                                + str(year)
                            )
                except Exception as e:
                    self.logger.critical(
                    ...
                    self.metaReq["year"] = (
                        "ERROR: The year provided in the metadata was invalid (Exception in log)."
                    )
            ...
            if "boundary type" in key.lower() and "name" not in key.lower():
                try:
                    ...
                    if val.upper().replace(" ", "") in validTypes:
                        self.logger.info(
                            "Valid Boundary Type detected: " + str(val) + "."
                        )
                        ...
                        self.metaReq["bType"] = (
                            "ERROR: The boundary type in the meta.txt file in invalid."
                        )
                except Exception as e:
                    self.logger.critical(
                    ...
                    self.metaReq["bType"] = (
                        "ERROR: The boundary type in the meta.txt file was invalid (Exception in log)."
                    )
            ...
            if "iso" in key.lower().strip():
                if len(val) != 3:
                    ...
                    self.metaReq["iso"] = (
                        "ERROR: ISO is invalid - we expect a 3-character ISO code following ISO-3166-1 (Alpha 3)."
                    )
                    ...
                    self.metaReq["iso"] = (
                        "ERROR: ISO is not on our list of valid ISO-3 codes."
                    )
                else:
            ...
            if "license" == key.lower():
                ...
                if val.lower().strip() not in self.validLicense:
                    ...
                    self.metaReq["license"] = "ERROR: Invalid license detected: " + str(
                        val
                    )
                else:
                ...
                if len(val.replace(" ", "")) > 0:
                    if val.lower() not in ["na", "nan", "null"]:
                        ...
                        if licPic != 0:
                            ...
                            self.logger.warning(
                                "No license image found. We check for license.png and license.jpg."
                            )
                            self.metaReq["licenseImage"] = "None Available"
                    else:
                        self.logger.critical("No license source detected.")
                        self.metaReq["licenseImage"] = (
                            "ERROR: No license source detected."
                        )
                else:
                    self.logger.critical("No license source detected.")
            ...
            if "link to source data" in key.lower():
                if len(val.replace(" ", "")) > 0:
                    if val.lower() not in ["na", "nan", "null"]:
                        ...
                        self.metaReq["dataSource"] = (
                            "ERROR: No link to source data found."
                        )
            ...
            if "other notes" in key.lower():
                if len(val.replace(" ", "")) > 0:
                    ...
                    self.logger.warning(
                        "No other notes detected.  This field is optional."
                    )
        ...
        for d in self.metaReq:
            ...
            if "ERROR" in self.metaReq[d]:
                self.logger.critical(
                    str(d) + "FAILED - ERROR IN DATA." + str(self.metaReq[d])
                )
    ...
    def checkBuildGeometryFiles(self):
        ...
        if len(nameCol) == 1:
            ...
            try:
                ...
                nameValues = (
                    self.geomDta[
                        self.geomDta[str(nameCol[0])].str.contains(".*", regex=True)
                    ][str(nameCol[0])]
                ).count()
                self.geomReq["names"] = str(nameCol[0])
                self.logger.info(
                    "Names: " + str(nameValues) + " | Example: " + str(nameExample)
                )
            except Exception as e:
                self.logger.warning(
                    "No name values were found, even though a column was present."
                )
                self.geomReq["names"] = str(nameCol[0])
        ...
        if len(nameICol) == 1:
            ...
            try:
                ...
                nameValues = (
                    self.geomDta[
                        self.geomDta[str(nameICol[0])].str.contains(".*", regex=True)
                    ][str(nameICol[0])]
                ).count()
                self.geomReq["iso"] = str(nameICol[0])
                self.logger.info(
                    "ISOs: " + str(nameValues) + " | Example: " + str(nameExample)
                )
            except Exception as e:
                self.logger.warning(
                    "No ISO values were found, even though a column was present."
                )
                self.geomReq["iso"] = str(nameICol[0])
        else:
            self.logger.warning(
                "No column for boundary ISOs found.  This is not required."
            )
        ...
        try:
            if self.geomDta.crs == "epsg:4326":
                ...
                self.geomReq["projection"] = "Projection confirmed as " + str(
                    self.geomDta.crs
                )
            else:
                self.logger.critical(
                    "The projection must be EPSG 4326.  The file proposed has a projection of: "
                    + str(self.geomDta.crs)
                )
                self.geomReq["projection"] = (
                    "ERROR: The projection must be EPSG 4326.  The file proposed has a projection of: "
                    + str(self.geomDta.crs)
                )
        except:
        ...
        for d in self.geomReq:
            ...
            if "ERROR" in self.geomReq[d]:
                self.logger.critical(
                    str(d) + " FAILED - ERROR IN DATA." + str(self.geomReq[d])
                )
        ...
        if allValid == 1:
            ...
            try:
                ...
                self.geomDta = self.geomDta.drop(
                    columns=[c for c in self.geomDta if c not in keepCols]
                )
    ...
    def calculateGeomMeta(self):
        ...
        try:
            ...
            self.logger.critical(
                "Failed to calculate number of administrative units: " + str(e)
            )
            return "ERROR: Failed to calculate number of administrative units: " + str(
                e
            )
        ...
        try:
            ...
            self.logger.critical(
                "Geometry statistics calculation error during vertices calculations: "
                + str(e)
            )
            return (
                "ERROR: Something went wrong calculating the geometric statistics (vertices) for the metadata: "
                + str(e)
            )
        ...
        try:
            ...
            self.logger.info(
                "Mean Perimeter: " + str(self.metaDataLib["meanPerimeterLengthKM"])
            )

        except Exception as e:
            self.logger.critical(
                "Geometry statistics calculation error during perimeter calculations: "
                + str(e)
            )
            return (
                "ERROR: Something went wrong calculating the geometric statistics (perimeter) for the metadata: "
                + str(e)
            )
        ...
        try:
            ...
            self.logger.critical(
                "Geometry statistics calculation error during area calculations: "
                + str(e)
            )
            return (
                "ERROR: Something went wrong calculating the geometric statistics (area) for the metadata: "
                + str(e)
            )
    ...
    def checkChange(self, metaTXT):
        ...
        try:
            ...
            self.logger.info(
                "New Size: " + str(newSize) + " | Old Size: " + str(oldSize)
            )
            ...
            self.logger.warning(
                "No files existed, or at least one file caused an error in the change detection."
            )
    ...
    def constructFiles(self):
            ...
            for i in self.metaDataLib:
                ...
                if i == "boundaryID":
                    ...
                    if (
                        ...
                        in self.metaDataLib["boundaryLicense"]
                    ):
                        out = (
                            out
                            + "3. In addition, the source of this file further requires attribution as per the OSM \n"
                        )
                ...
                if i == "boundaryISO":
                    out = "ISO-3166-1 (Alpha-3): " + str(
                        self.metaDataLib["boundaryISO"]
                    )
                if i == "boundaryYear":
                    out = "Boundary Representative of Year: " + str(
                        self.metaDataLib["boundaryYear"]
                    )
                if i == "boundaryType":
                    out = "Boundary Type: " + str(self.metaDataLib["boundaryType"])
                if i == "boundarySource":
                    out = "Boundary Source(s): " + str(
                        self.metaDataLib["boundarySource"]
                    )
                if i == "boundaryCanonical":
                    out = "Canonical Boundary Type Name: " + str(
                        self.metaDataLib["boundaryCanonical"]
                    )
                ...
                if i == "sourceDataUpdateDate":
                    out = "Source Data Updated On: " + str(
                        self.metaDataLib["sourceDataUpdateDate"]
                    )
                if i == "buildDate":
                    out = "File Built On: " + str(self.metaDataLib["buildDate"])
                if i == "admUnitCount":
                    out = "Number of Administrative Units: " + str(
                        self.metaDataLib["admUnitCount"]
                    )
        ...
        if self.changesDetected == True:
            ...
            removeNames = [
                "CITATION-AND-USE-geoBoundaries-gbOpen.txt",
                "CITATION-AND-USE-geoBoundaries-gbAuthoritative.txt",
                "CITATION-AND-USE-geoBoundaries-gbOpen.txt",
            ]
            ...
            self.logger.info(
                "No changes in source detected, not generating release output files."
            )
