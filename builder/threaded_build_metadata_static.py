...
def fast_copy(src, dst):
    ...
    try:
        if os.path.exists(src) and os.path.exists(dst) and os.path.samefile(src, dst):
            with log_lock:
                logging.warning(
                    f"COPY: Source and destination are the same file ({src}), skipping copy."
                )
            print(
                f"COPY: Source and destination are the same file ({src}), skipping copy."
            )
    ...
    try:
        with connect_to_db() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    'SELECT "STATUS", "TIME" FROM status WHERE "STATUS_TYPE" = %s ORDER BY "TIME" DESC LIMIT 1',
                    ("METADATA_STATIC_STATUS",),
                )
                row = cur.fetchone()
    ...
    try:
        with connect_to_db() as conn:
            with conn.cursor() as cur:
                # Try update first
                cur.execute(
                    'UPDATE status SET "STATUS" = %s, "TIME" = NOW() WHERE "STATUS_TYPE" = %s',
                    (status_msg, "METADATA_STATIC_STATUS"),
                )
                if cur.rowcount == 0:
                    # No row to update, insert
                    cur.execute(
                        'INSERT INTO status ("STATUS_TYPE", "STATUS", "TIME") VALUES (%s, %s, NOW())',
                        ("METADATA_STATIC_STATUS", status_msg),
                    )
                conn.commit()
    except Exception as e:
        with log_lock:
...
def get_full_db_build_status():
    try:
        with connect_to_db() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    'SELECT "STATUS" FROM status WHERE "STATUS_TYPE" = %s',
                    ("FULL_DB_BUILD",),
                )
                row = cur.fetchone()
...
def normalize_meta_dict(meta):
    ...
    for k, v in meta.items():
        ...
        if isinstance(v, str):
            v_clean = (
                v.replace('"', "").replace("'", "").replace(",", "").strip().lower()
            )
...
def process_metadata(args):
    ...
    with log_lock:
        logging.info(
            f"Thread {thread_name} started: path={path}, metaSearch={metaSearch}"
        )
    ...
    try:
        ...
        if isoMeta.empty:
            with log_lock:
                logging.error(
                    f"isoMeta lookup failed: path={path}, meta['boundaryISO']={meta.get('boundaryISO')}, isoMeta is empty. isoDetails Alpha-3code values: {isoDetails['Alpha-3code'].unique()}"
                )
        ...
        try:
            ...
            with log_lock:
                logging.error(
                    f"Error accessing isoMeta['Name'].values[0]: path={path}, meta['boundaryISO']={meta.get('boundaryISO')}, error={e}, isoMeta={isoMeta}"
                )
            raise
        ...
        meta["licenseDetail"] = (
            meta["licenseDetail"].replace(",", "").replace("\\", "").replace('"', "")
        )
        metaLine = (
            metaLine
            + meta["boundarySource"]
        ...
        metaLine = (
            metaLine
            + isoMeta["Continent"].values[0]
            + '","'
            + isoMeta["UNSDG-region"].values[0]
            + '","'
        )
        metaLine = (
            metaLine
            + isoMeta["UNSDG-subregion"].values[0]
            + '","'
        ...
        metaLine = (
            metaLine
            + str(meta["admUnitCount"])
            + '","'
            + str(meta["meanVertices"])
            + '","'
            + str(meta["minVertices"])
            + '","'
            + str(meta["maxVertices"])
            + '","'
        )
        metaLine = (
            metaLine
            + str(meta["meanPerimeterLengthKM"])
            + '","'
            + str(meta["minPerimeterLengthKM"])
            + '","'
            + str(meta["maxPerimeterLengthKM"])
            + '","'
        )
        metaLine = (
            metaLine
            + str(meta["meanAreaSqKM"])
            + '","'
            + str(meta["minAreaSqKM"])
            + '","'
            + str(meta["maxAreaSqKM"])
            + '",'
        )
        ...
        static_file_path = (
            path
            + "/"
            + f"geoBoundaries-{meta['boundaryISO']}-{meta['boundaryType']}-all.zip"
        )
        ...
        dest_path = (
            "/sciclone/geograd/geoBoundaries/geoBoundaryBot/gbWeb/data/static/"
            + static_link_file
        )
        ...
        if need_copy:
            ...
            if os.path.exists(source_dir):
                ...
                for filename in os.listdir(source_dir):
                    ...
                    try:
                        ...
                        with log_lock:
                            logging.error(
                                f"Failed to copy {source_file} to {target_file}: {e}"
                            )
            else:
                with log_lock:
            ...
            with log_lock:
                logging.info(
                    f"HASH: Copying static file to {dest_path} (does not exist, hash in name does not match, or zip invalid)"
                )
            try:
        ...
        with log_lock:
            logging.info(
                f"Thread {thread_name}: upserting metadata for ISO={meta.get('boundaryISO', '?')}, ADM={meta.get('boundaryType', '?')}"
            )
                ...
                with conn.cursor() as cur:
                    ...
                    """,
                        (
                            ...
                            float(meta["meanVertices"])
                            if meta["meanVertices"]
                            else None,
                            int(meta["minVertices"]) if meta["minVertices"] else None,
                            int(meta["maxVertices"]) if meta["maxVertices"] else None,
                            float(meta["meanPerimeterLengthKM"])
                            if meta["meanPerimeterLengthKM"]
                            else None,
                            float(meta["maxPerimeterLengthKM"])
                            if meta["maxPerimeterLengthKM"]
                            else None,
                            float(meta["minPerimeterLengthKM"])
                            if meta["minPerimeterLengthKM"]
                            else None,
                            float(meta["meanAreaSqKM"])
                            if meta["meanAreaSqKM"]
                            else None,
                    ...
                    with log_lock:
                        logging.info(
                            f"Thread {thread_name}: Upsert committed for ISO={meta.get('boundaryISO', '?')}, ADM={meta.get('boundaryType', '?')} (DB: {db_upsert_time:.3f}s)"
                        )
        except Exception as e:
            db_upsert_time = (
                time.time() - db_upsert_start
            )  # Capture timing even on error
            with log_lock:
                logging.error(
                    f"Failed to upsert meta for {meta['boundaryISO']}-{meta['boundaryType']}: {e} (DB time: {db_upsert_time:.3f}s)"
                )

        thread_total_time = time.time() - thread_start_time
        with log_lock:
            logging.info(
                f"Thread {thread_name} completed in {thread_total_time:.3f}s (JSON: {json_load_time:.3f}s, ISO: {iso_lookup_time:.3f}s, STR: {string_processing_time:.3f}s, CHK: {file_check_time:.3f}s, CPY: {file_copy_time:.3f}s, DB: {db_upsert_time:.3f}s)"
            )
...
def main():
    ...
    if ready_count > 0:
        with log_lock:
            logging.info(
                f"{ready_count} tasks are in 'ready' state. Metadata processing will not proceed."
            )
        update_metadata_static_status(
            f"Not started: {ready_count} tasks are in 'ready' state."
        )
        return
    ...
    current_time = datetime(
        2025, 8, 12, 15, 35, 4, tzinfo=pytz.timezone("America/New_York")
    )
    ...
    if last_time:
        ...
        if elapsed < 48 * 3600:
            with log_lock:
                logging.info(
                    f"Metadata processing ran less than 48 hours ago (last at {last_time_dt}). Not running again."
                )
            return
    ...
    status_thread = threading.Thread(
        target=periodic_status_updater, args=(stop_status_thread, status_func, 300)
    )
    status_thread.start()
    ...
    with log_lock:
        logging.info(
            f"TIMING: Boundary meta table creation took {step4_time:.2f} seconds"
        )
    ...
    with ThreadPoolExecutor(max_workers=30) as executor:
        with log_lock:
            logging.info(
                f"Launching ThreadPoolExecutor for metadata processing with {len(meta_tasks)} tasks..."
            )
        ...
        with log_lock:
            logging.info(
                f"TIMING: All thread processing took {processing_time:.2f} seconds"
            )
