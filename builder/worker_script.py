...
log_file = os.path.join(
    LOG_DIR, f"worker_script_{iso}_{adm}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
)
logging.basicConfig(
    level=logging.INFO,
    ...
    try:
                    ...
                    try:
                        ...
                        for encoding in encodings:
                            try:
                                ...
                                logging.warning(
                                    f"Failed to read file with encoding: {encoding}"
                                )
                                continue

                        if content is None:
                            logging.error(
                                f"Could not read file with any encoding: {meta_path}"
                            )
                            continue

                        logging.info(
                            f"Metadata content: {content[:200]}..."
                        )  # Log first 200 chars
                        ...
                        try:
                            ...
                            logging.warning(
                                f"JSON parsing error: {json_err}. Trying to read file line by line..."
                            )
                            # Try to extract sourceDataUpdateDate manually using regex
                            import re

                            source_date_match = re.search(
                                r'"sourceDataUpdateDate"\s*:\s*"([^"]+)"', content
                            )
                            if source_date_match:
                                source_date_str = source_date_match.group(1)
                                logging.info(
                                    f"Extracted sourceDataUpdateDate using regex: {source_date_str}"
                                )
                                try:
                                    ...
                                    for fmt in [
                                        ...
                                        try:
                                            ...
                                            logging.info(
                                                f"Successfully parsed source date as: {source_date}"
                                            )
                                            break
                                        except ValueError:
                                            continue
                                    ...
                                    if not source_date:
                                        try:
                                            ...
                                            if (
                                                ...
                                                weekday, month, day, time_str, year = (
                                                    parts
                                                )
                                                month_map = {
                                                    "Jan": 1,
                                                ...
                                                if month_num:
                                                    ...
                                                    logging.info(
                                                        f"Successfully parsed source date using manual parsing: {source_date}"
                                                    )
                                        except Exception as e:
                                            logging.warning(
                                                f"Error in manual date parsing: {e}"
                                            )
                                except Exception as e:
                                    logging.warning(
                                        f"Error parsing source date '{source_date_str}': {e}"
                                    )
                        ...
                        if "sourceDataUpdateDate" in meta_data:
                            ...
                            logging.info(
                                f"Found sourceDataUpdateDate in metadata: {source_date_str}"
                            )
                            try:
                                # Try different date formats
                                for fmt in [
                        ...
                        if not source_date:
                            ...
                            for field in date_fields:
                                ...
                                if source_date_str:
                                    logging.info(
                                        f"Found source date in field '{field}': {source_date_str}"
                                    )
                                    try:
                                        # Try different date formats
                                        for fmt in [
        ...
        with connect_to_db() as conn:
            with conn.cursor() as cur:
                ...
                logging.info(
                    f"Inserting values - STATUS_TYPE: {status_type}, STATUS: {status}, TIME: {timestamp}, SOURCE_DATE: {source_date}"
                )
                ...
                # Verify the update
                cur.execute(
                    'SELECT "SOURCE_DATE" FROM worker_status WHERE "STATUS_TYPE" = %s',
                    (status_type,),
                )
                result = cur.fetchone()
                logging.info(
                    f"Verified SOURCE_DATE after update for {status_type}: {result[0] if result else None}"
                )
    except Exception as e:
...
countries = pd.read_csv(
    os.path.join(os.path.dirname(__file__), META_DIR + "iso_3166_1_alpha_3.csv")
)
isoList = countries["Alpha-3code"].values

licenses = pd.read_csv(
    os.path.join(os.path.dirname(__file__), META_DIR + "gbLicenses.csv")
)
...
try:
    ...
    try:
        ...
        # Initialize builder
        bnd = builder(
            iso, adm, "gbOpen", GB_DIR, LOG_DIR, TMP_DIR, isoList, licenseList
        )
        ...
        for stage_method, stage_status in stages:
            ...
            if "ERROR" in str(result):
                ...
                update_status_in_db(
                    status_type, f"ERROR_{stage_status}", datetime.now()
                )
                ...
                with connect_to_db() as conn:
                    with conn.cursor() as cur:
                        ...
                        """
                        cur.execute(
                            update_query, (datetime.now(), str(result), iso, adm)
                        )
                        conn.commit()
                build_results.append(
                    [iso, adm, "gbOpen", result, f"E{stage_status[0]}"]
                )
                break
        else:
            ...
            with connect_to_db() as conn:
                with conn.cursor() as cur:
                    ...
                    update_status_in_db(
                        status_type, f"COMPLETE: {elapsed_time}", datetime.now()
                    )
                    # Update tasks table
                    update_query = """
                    UPDATE tasks
                    ...
                    cur.execute(
                        update_query,
                        (datetime.now(), f"Completed in {elapsed_time}", taskid),
                    )
                    conn.commit()
    ...
    if build_results:
        ...
        if len(result) >= 5 and result[4].startswith(
            ...
            final_status = (
                f"ERROR_{result[4]}: {result[3]}"  # Include both error code and message
            )
        else:
    ...
    update_status_in_db(
        status_type, f"WORKER_SCRIPT_ERROR: {str(main_error)}", datetime.now()
    )
