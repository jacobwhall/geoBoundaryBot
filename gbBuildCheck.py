...
def findDate(queryResponse):
    for i in range(
        0, len(queryResponse["data"]["repository"]["object"]["blame"]["ranges"])
    ):
        curDate = queryResponse["data"]["repository"]["object"]["blame"]["ranges"][i][
            "commit"
        ]["committedDate"]
        if i == 0:
            recentDate = curDate
...
try:
    ...
    try:
        ...
        if buildDate > commitDate:
            print(
                "Build is already up-to-date.  Confirming build script has not updated."
            )
        ...
        print(
            "No build file for this layer currently exists in the repository.  Commencing new build."
        )
