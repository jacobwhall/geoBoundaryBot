...
if len(ws["zips"]) > 0:
    gbHelpers.logWrite(
        checkType, "Modified zip files found.  Downloading and checking validity."
    )
    ...
    for z in ws["zips"]:
        ...
        if "meta.txt" in bZip.namelist():
            ...
            gbHelpers.logWrite(
                checkType, "CRITICAL ERROR: Metadata file does not exist in " + z
            )
            gbHelpers.gbEnvVars(
                "RESULT",
        ...
        if len(allShps) == 1:
            if len(shp) == 1:
                gbHelpers.logWrite(
                    checkType,
                    "Shapefile (*.shp) found. Checking if all required files are present.",
                )
                if len(list(filter(lambda x: x[-4:] == ".shx", bZip.namelist()))) < 1:
                    gbHelpers.logWrite(
                        checkType,
                        "CRITICAL ERROR: A valid *.shp requires a *.shx (index) file. None was found in "
                        + z,
                    )
                    gbHelpers.gbEnvVars(
                        "RESULT",
                ...
                if len(list(filter(lambda x: x[-4:] == ".dbf", bZip.namelist()))) < 1:
                    gbHelpers.logWrite(
                        checkType,
                        "CRITICAL ERROR: A valid *.shp requires a *.dbf (index) file. None was found in "
                        + z,
                    )
                    gbHelpers.gbEnvVars(
                        "RESULT",
                ...
                if len(list(filter(lambda x: x[-4:] == ".prj", bZip.namelist()))) < 1:
                    gbHelpers.logWrite(
                        checkType,
                        "CRITICAL ERROR: A valid *.shp requires a *.prj (index) file. None was found in "
                        + z,
                    )
                    gbHelpers.gbEnvVars(
                        "RESULT",
        ...
        if len(allShps) == 0:
            gbHelpers.logWrite(
                checkType, "CRITICAL ERROR: No *.shp or *.geojson found for " + z
            )
            gbHelpers.gbEnvVars(
                "RESULT",
        ...
        if checkFail == 1:
            ...
            gbHelpers.logWrite(
                checkType,
                "CRITICAL ERROR: Zipfile validity checks failed for "
                + z
                + ".  Check the log to see what is wrong.",
            )
        else:
    ...
    if zipFailures > 0:
        gbHelpers.logWrite(
            checkType,
            "CRITICAL ERROR: At least one file check failed; check the log to see what's wrong.",
        )
    else:
        print("Set")
