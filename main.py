...
@click.command()
@click.option("--build-type", "-b", required=True,
              help="Release type (e.g., gbOpen, gbHumanitarian, gbAuthoritative).")
@click.option("--build-ver", "-v", required=True,
              type=click.Choice(["nightly", "release"], case_sensitive=False),
              help="Build version.")
@click.option("--countries", "-c", required=True,
              help="Comma-separated ISO country codes to process.")
@click.option("--type-query", "-t", required=True,
              help="Boundary type filter (e.g., ADM0, ADM1).")
@click.option("--api-key", "-k", envvar="GITHUB_TOKEN", required=True,
