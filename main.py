...
@click.option(
    ...
    "-b",
    required=True,
    help="Release type (e.g., gbOpen, gbHumanitarian, gbAuthoritative).",
)
@click.option(
    "--build-ver",
    "-v",
    required=True,
    type=click.Choice(["nightly", "release"], case_sensitive=False),
    help="Build version.",
)
@click.option(
    "--countries",
    "-c",
    required=True,
    help="Comma-separated ISO country codes to process.",
)
@click.option(
    "--type-query", "-t", required=True, help="Boundary type filter (e.g., ADM0, ADM1)."
)
@click.option(
    "--api-key",
    "-k",
...
def main(build_type, build_ver, countries, type_query, api_key):
        ...
        for filename in filesToProcess:
            ...
            if row["status"] == "FAIL":
                ...
                if issueCount == 1 and repo_create == False and comment_create == False:
                    ...
                    if (
                        ...
                        print(
                            "I have already commented on " + filename + "+" + buildType
                        )
            ...
            if (
                ...
                for i in range(
                    ...
                    curDate = response["data"]["repository"]["object"]["blame"][
                        "ranges"
                    ][i]["commit"]["committedDate"]
                    print(curDate)
                    print(i)
                    if i == 0:
                        commitDate = curDate
                ...
                with zipfile.ZipFile(ws["working"] + "/" + ws["zips"][0]) as zF:
                    ...
                    geojson = list(
                        filter(lambda x: x[-8:] == ".geojson", zF.namelist())
                    )
                ...
                dta.to_file(
                    workingPath + row["boundaryID"] + ".geoJSON", driver="GeoJSON"
                )
    ...
    try:
        keys = csvR[0].keys()
        with open(
            os.path.expanduser("~") + "/artifacts/results" + str(buildType) + ".csv",
            "w",
        ) as f:
